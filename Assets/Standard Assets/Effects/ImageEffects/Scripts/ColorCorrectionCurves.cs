...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class ColorCorrectionCurves : PostEffectsBase
	{
        ...
        private float discoSpeed = 0f;
        private double discoTimer = 0.0;
        private double discoNextColorChange = 0.0;
        private double discoStopTime = 0.0;
        ...
        {
            return AnimationCurve.Linear(UnityEngine.Random.Range(0f, 0.15f), UnityEngine.Random.Range(0f, 0.15f),
                                             UnityEngine.Random.Range(0.85f, 1f), UnityEngine.Random.Range(0.85f, 1f));
        }
        ...
        public void DiscoStart(float speed, float stopTime)
        {
            discoSpeed = speed;
            discoNextColorChange = discoTimer;
            discoStopTime = discoTimer + stopTime;
        }

        public void DiscoStop()
        {
            discoSpeed = 0f;

            AnimationCurve linearCurve = AnimationCurve.Linear(0f,0f,1f,1f);
            redChannel = greenChannel = blueChannel = linearCurve;
            UpdateTextures();
        }

        private void Update()
        {
            if (discoSpeed > 0.01f && Time.timeScale > 0f)
            {
                discoTimer += 0.01666666666666666666666 * Time.timeScale;
                while (discoTimer >= discoNextColorChange)
                {
                    discoNextColorChange += 1f / discoSpeed;
                    redChannel = GenerateRandomCurve();
                    greenChannel = GenerateRandomCurve();
                    blueChannel = GenerateRandomCurve();
                    UpdateTextures();
                }
                if (discoTimer >= discoStopTime)
                {
                    DiscoStop();
                }
            }
