	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				int frame = frac(0.5*_Time.z*_AnimSpeed/_CelCount)*_CelCount;
				tx /= _CelCount;
				tx += float(frame) / _CelCount;

			    fixed4 c = SampleSpriteTexture(float2(tx,IN.texcoord.y)) * _Color;//UNITY_ACCESS_INSTANCED_PROP(Props, _Color);
