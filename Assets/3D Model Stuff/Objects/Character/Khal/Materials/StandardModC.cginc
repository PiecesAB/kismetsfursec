...
#ifndef UNITY_STANDARD_CORE_INCLUDED_MOD
#define UNITY_STANDARD_CORE_INCLUDED_MOD

#include "UnityCG.cginc"
#include "UnityShaderVariables.cginc"
#include "UnityStandardConfig.cginc"
#include "UnityStandardInputMod.cginc"
#include "UnityPBSLightingMod.cginc"
...
inline FragmentCommonData FragmentSetup(float4 i_tex, half3 i_eyeVec, half3 i_viewDirForParallax, half4 tangentToWorld[3], half3 i_posWorld)
{
	...
	{
		float3 rad;
		float tmod = (_Time.x*_Rainbow+sin(i_posWorld.x*0.05)+cos(i_posWorld.y*0.05)- sin(i_posWorld.z*0.05)+3.0) % 1.0;
		rad.r = max(max(0.33333333 - abs(tmod), 0.33333333 - abs(tmod - 1.0)), 0.0);
		rad.g = max(0.33333333 - abs(tmod - 0.33333333), 0.0);
		rad.b = max(0.33333333 - abs(tmod - 0.66666666), 0.0);
		rad.rgb = min(rad.rgb, 0.16666666)*6.0;
		_Color *= float4(rad, 1.0);
	}
...
VertexOutputForwardBase vertForwardBase(VertexInput v) //orthographic camera
{
...
#endif
	//_FakeZ = float4(14.0, -200.0, 0.0, 0.0);
	...
	//float4 q = mul(UNITY_MATRIX_P, p);
	//o.pos = q;//float4(round(q.xy), q.z, q.w);
	float4 rand1 = sin(5732.28736*cos(38743.9382*(v.vertex.x + v.vertex.y - v.vertex.z + _Time.z)));
	o.pos = mul(UNITY_MATRIX_P, float4(round(p.xyz / _IntVScale)*_IntVScale, p.w)) + _Fizz*rand1;
	//float zd = length(mul(UNITY_MATRIX_MV, v.vertex));
	//float4 fakeVertex = mul(unity_WorldToObject, mul(unity_CameraToWorld, o.pos));
	o.tex = TexCoords(v);
	o.zd = o.pos.z;
	//float3 fakeWorld = mul(unity_CameraToWorld, o.pos).xyz;
	o.eyeVec = NormalizePerVertexNormal(/*fakeWorld.xyz*/posWorld.xyz - _WorldSpaceCameraPos);
